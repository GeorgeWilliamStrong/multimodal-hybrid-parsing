...
def test_pdf_parsing():
    ...
    parser = DocumentParser(
        ...
        images_scale=150/72.0
    )

    # Get all files from samples directory
    samples_dir = Path("tests/samples")
    output_dir = Path("tests/output/markdown/heuristic_cuda_img_desc_granite_half_scale")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Create timing stats directory
    stats_dir = Path("tests/stats")
    stats_dir.mkdir(parents=True, exist_ok=True)
    benchmark_file = stats_dir / "heuristic_parsing_cuda_img_desc_granite_half_scale_stats.json"
